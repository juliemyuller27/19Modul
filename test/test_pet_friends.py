...
from setting import (valid_email,
                      valid_password,
                      not_valid_email,
                      not_valid_password)
...
def test_get_all_pets_with_valid_key(filter=''):
    """ Отправляем запрос списка всех питомцев возвращает не пустой список.
      1.получаем api ключ и сохраняем в переменную auth_key.
      2.запрашиваем список всех питомцев.
...
def test_add_new_pet_with_blank_age(
    name='Иллюзион',
    animal_type='Кот',
    age='',
    pet_photo_path='images/cat1.jpg'
...
def test_add_new_pet_with_incorrect_age(
    ...
    """ Проверяем, что запрос на добавление нового питомца
        с некорректным параметром
            возраст питомца = 555555555555555
        выполняется успешно."""
    pet_photo_path = os.path.join(os.path.dirname(__file__), pet_photo_path)
...
def test_successful_delete_self_pet():
    ...
    if not my_pets['pets']:
        pf.add_new_pet(auth_key, "Котенок", "Кошка", "2", "images/cat1.jpg")
        _, my_pets = pf.get_list_of_pets(auth_key, pf.MY_PETS)
...
def test_successful_update_self_pet_info(
    ...
    _, my_pets = pf.get_list_of_pets(auth_key, pf.MY_PETS)
    if not my_pets['pets']:
        raise Exception("He isn`t my pet")
    pet_id = my_pets['pets'][0]['id']
...
def test_rejection_update_self_pet_info_without_name(
    ...
    """ Проверяем что очистить имя питомца
        путём передачи пустого поля name невозможно """
...
def test_rejection_update_self_pet_info_without_animal_type(
    ...
    """ Проверяем что очистить типа питомца путём
        передачи пустого поля animal_type невозможно"""
...
def test_succsessful_update_self_pet_info_with_spase_name(
    ...
    """ Проверяем что очистка имени питомца путём передачи пробела
...
def test_add_new_pet_with_data_without_foto(
    name='Суетолог',
    animal_type='Котский',
    age='1'):
    """ Проверяем, что запрос на добавление нового питомца
...
def test_add_new_pet_with_incorrect_data_without_foto(
    ...
    """ Проверка, что запрос на добавление нового питомца
